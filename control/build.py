...
class MicrocontrollerConfigGenerator:
    ...
    def _generate_header_content(self, microcontroller_id: str, 
        ...
        lines = []
        lines.append(f"// autogen_config_{microcontroller_id}.h")
        ...
        lines.append("")
        lines.append(f"#ifndef AUTOGEN_CONFIG_{microcontroller_id.upper()}_H")
        lines.append(f"#define AUTOGEN_CONFIG_{microcontroller_id.upper()}_H")
        lines.append("")
        
        # Device identification
        lines.append("// === Device Identification ===")
        device_id = config.device_id or microcontroller_id
        lines.append(f'#define DEVICE_ID "{device_id}"')
        if config.description:
            lines.append(f'#define DEVICE_DESCRIPTION "{config.description}"')
        lines.append("")
        
        # Global settings (WiFi, MQTT, NTP)
        lines.append("// === Global Settings ===")
        global_settings = self.system_config.global_settings
        
        # WiFi settings (local overrides global)
        wifi_ssid = config.wifi.ssid if config.wifi and config.wifi.ssid else "WIFI_SSID_FROM_GLOBAL"
        wifi_password = config.wifi.password if config.wifi and config.wifi.password else "WIFI_PASSWORD_FROM_GLOBAL"
        lines.append(f'#define WIFI_SSID "{wifi_ssid}"')
        lines.append(f'#define WIFI_PASSWORD "{wifi_password}"')
        
        # MQTT settings (local overrides global)
        mqtt_address = (config.mqtt_broker.address if config.mqtt_broker and config.mqtt_broker.address 
                       else global_settings.mqtt_broker.address)
        mqtt_port = (config.mqtt_broker.port if config.mqtt_broker and config.mqtt_broker.port 
                    else global_settings.mqtt_broker.port)
        lines.append(f'#define MQTT_BROKER_ADDRESS "{mqtt_address}"')
        lines.append(f'#define MQTT_BROKER_PORT {mqtt_port}')
        
        if global_settings.mqtt_broker.username:
            mqtt_username = (config.mqtt_broker.username if config.mqtt_broker and config.mqtt_broker.username 
                           else global_settings.mqtt_broker.username)
            lines.append(f'#define MQTT_USERNAME "{mqtt_username}"')
            
        if global_settings.mqtt_broker.password:
            mqtt_password = (config.mqtt_broker.password if config.mqtt_broker and config.mqtt_broker.password 
                           else global_settings.mqtt_broker.password)
            lines.append(f'#define MQTT_PASSWORD "{mqtt_password}"')
        
        # NTP settings (local overrides global)
        ntp_address = (config.ntp_server.address if config.ntp_server and config.ntp_server.address 
                      else global_settings.ntp_server.address)
        ntp_offset = (config.ntp_server.utc_offset_seconds if config.ntp_server and config.ntp_server.utc_offset_seconds is not None
                     else global_settings.ntp_server.utc_offset_seconds)
        ntp_interval = (config.ntp_server.update_interval_ms if config.ntp_server and config.ntp_server.update_interval_ms 
                       else global_settings.ntp_server.update_interval_ms)
        lines.append(f'#define NTP_SERVER_ADDRESS "{ntp_address}"')
        lines.append(f'#define NTP_UTC_OFFSET_SECONDS {ntp_offset}')
        lines.append(f'#define NTP_UPDATE_INTERVAL_MS {ntp_interval}')
        lines.append("")
        
        # Timing constants
        lines.append("// === Timing Constants ===")
        if config.timing_constants:
            lines.append(f'#define PUBLISH_INTERVAL_MS {config.timing_constants.publish_interval_ms}')
            lines.append(f'#define WIFI_CONNECT_TIMEOUT_MS {config.timing_constants.wifi_connect_timeout_ms}')
            lines.append(f'#define MQTT_CONNECT_TIMEOUT_MS {config.timing_constants.mqtt_connect_timeout_ms}')
            lines.append(f'#define NTP_SYNC_TIMEOUT_MS {config.timing_constants.ntp_sync_timeout_ms}')
        elif config.publish_frequency_ms:  # Legacy fallback
            lines.append(f'#define PUBLISH_INTERVAL_MS {config.publish_frequency_ms}')
            lines.append('#define WIFI_CONNECT_TIMEOUT_MS 30000  // Default')
            lines.append('#define MQTT_CONNECT_TIMEOUT_MS 20000  // Default')
            lines.append('#define NTP_SYNC_TIMEOUT_MS 15000  // Default')
        
        if config.output_republish_frequency_ms:
            lines.append(f'#define OUTPUT_REPUBLISH_FREQUENCY_MS {config.output_republish_frequency_ms}')
        lines.append("")
        
        # Hardware points (new approach)
        if config.hardware_points:
            lines.append("// === Hardware Points (New Approach) ===")
            
            for hw_point in config.hardware_points:
                point_name = hw_point.name
                
                if hw_point.point_kind == "actuator":
                    lines.append(f"// Actuator: {point_name}")
                    lines.append(f'#define POINT_NAME_{point_name} "{point_name}"')
                    lines.append(f'#define UUID_{point_name}_WRITE "{hw_point.write_point_uuid_ref}"')
                    lines.append(f'#define UUID_{point_name}_READBACK "{hw_point.readback_point_uuid_ref}"')
                    lines.append(f'#define PIN_{point_name} {hw_point.pin}')
                    lines.append(f'#define MODE_{point_name} {hw_point.pin_mode}')
                    lines.append(f'#define INITIAL_STATE_{point_name} {hw_point.initial_state}')
                    
                    # Generate MQTT topics
                    write_point = points_by_uuid.get(hw_point.write_point_uuid_ref)
                    readback_point = points_by_uuid.get(hw_point.readback_point_uuid_ref)
                    if write_point:
                        write_topic = topic_generator.generate_topic_for_point(write_point)
                        lines.append(f'#define TOPIC_{point_name}_WRITE "{write_topic}"')
                    if readback_point:
                        readback_topic = topic_generator.generate_topic_for_point(readback_point)
                        lines.append(f'#define TOPIC_{point_name}_READBACK "{readback_topic}"')
                    lines.append("")
                    
                elif hw_point.point_kind == "system_info":
                    lines.append(f"// System Info: {point_name}")
                    lines.append(f'#define POINT_NAME_{point_name} "{point_name}"')
                    lines.append(f'#define UUID_{point_name}_DATA "{hw_point.data_point_uuid_ref}"')
                    
                    # Generate MQTT topic
                    data_point = points_by_uuid.get(hw_point.data_point_uuid_ref)
                    if data_point:
                        data_topic = topic_generator.generate_topic_for_point(data_point)
                        lines.append(f'#define TOPIC_{point_name}_DATA "{data_topic}"')
                    lines.append("")
                    
                elif hw_point.point_kind == "sensor_data":
                    lines.append(f"// Sensor Data: {point_name}")
                    lines.append(f'#define POINT_NAME_{point_name} "{point_name}"')
                    lines.append(f'#define UUID_{point_name}_DATA "{hw_point.data_point_uuid_ref}"')
                    if hw_point.pin is not None:
                        lines.append(f'#define PIN_{point_name} {hw_point.pin}')
                    if hw_point.pin_mode:
                        lines.append(f'#define MODE_{point_name} {hw_point.pin_mode}')
        ...
        if config.digital_outputs:
            legacy_sections.append("// === Digital Outputs (Legacy) ===")
            for output in config.digital_outputs:
                output_name = output.name or f"OUTPUT_{output.pin}"
                legacy_sections.append(f"// Digital Output: {output_name}")
                legacy_sections.append(f'#define PIN_{output_name} {output.pin}')
                legacy_sections.append(f'#define UUID_{output_name} "{output.point_uuid}"')
                legacy_sections.append(f'#define INITIAL_STATE_{output_name} {output.initial_state.upper() if output.initial_state else "UNINITIALIZED"}')
                
                # Generate MQTT topic
                point = points_by_uuid.get(output.point_uuid)
                if point:
                    topic = topic_generator.generate_topic_for_point(point)
                    legacy_sections.append(f'#define TOPIC_{output_name} "{topic}"')
                legacy_sections.append("")
        
        if config.i2c:
            legacy_sections.append("// === I2C Configuration (Legacy) ===")
            legacy_sections.append(f'#define I2C_SDA_PIN {config.i2c.sda_pin}')
            legacy_sections.append(f'#define I2C_SCL_PIN {config.i2c.scl_pin}')
            if config.i2c.clock_speed:
                legacy_sections.append(f'#define I2C_CLOCK_SPEED {config.i2c.clock_speed}')
            legacy_sections.append("")
        
        if config.i2c_devices:
            legacy_sections.append("// === I2C Devices (Legacy) ===")
            for i, device in enumerate(config.i2c_devices):
                device_name = f"I2C_DEVICE_{i}_{device.sensor_model}"
                legacy_sections.append(f"// I2C Device: {device.sensor_model}")
                legacy_sections.append(f'#define {device_name}_MODEL "{device.sensor_model}"')
                legacy_sections.append(f'#define {device_name}_ADDRESS {device.address}')
                for measurement, uuid in device.point_uuids.items():
                    legacy_sections.append(f'#define UUID_{device_name}_{measurement.upper()} "{uuid}"')
                    point = points_by_uuid.get(uuid)
                    if point:
                        topic = topic_generator.generate_topic_for_point(point)
                        legacy_sections.append(f'#define TOPIC_{device_name}_{measurement.upper()} "{topic}"')
                legacy_sections.append("")
        
        # Add legacy sections if any exist
        if legacy_sections:
            lines.extend(legacy_sections)
        
        # Close header guard
        lines.append(f"#endif // AUTOGEN_CONFIG_{microcontroller_id.upper()}_H")
        lines.append("")
